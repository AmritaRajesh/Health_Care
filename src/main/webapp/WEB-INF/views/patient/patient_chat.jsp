...
<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Chat - Patient</title>
...
.contact-item{padding:14px 16px;border-bottom:1px solid #f1f5f9;cursor:pointer;transition:.2s;text-decoration:none;display:block;color:inherit;}
...
<h1 class="page-title">Chat with Doctors</h1>
<p class="page-subtitle">Direct messaging with your healthcare providers</p>

<div class="chat-container">
  <div class="contacts">
    <div style="padding:14px 16px;border-bottom:1px solid #e2e8f0;background:#f8fafc;"><strong class="text-dark">My Doctors</strong></div>
    <c:forEach var="entry" items="${doctorMap}">
      <a href="${pageContext.request.contextPath}/patient/chat?with=${entry.key}" class="contact-item ${chatWithId == entry.key ? 'active' : ''}">
        <div class="contact-name">Dr. ${entry.value}</div>
        <div class="contact-role">doctor</div>
      </a>
    </c:forEach>
    <c:forEach var="ct" items="${contacts}">
      <a href="${pageContext.request.contextPath}/patient/chat?with=${ct.id}" class="contact-item ${chatWithId == ct.id ? 'active' : ''}">
    ...
    <c:if test="${empty contacts && empty doctorMap}"><div class="text-center text-muted p-4 small">No doctors yet. Book an appointment first.</div></c:if>
  </div>

  <div class="chat-area">
    <c:if test="${empty chatWith}">
      <div class="empty-state"><i class="fa-regular fa-comments fs-1 mb-3"></i><h5>Select a doctor to start chatting</h5></div>
    </c:if>
    <c:if test="${not empty chatWith}">
      <div class="chat-header"><strong class="text-dark">Dr. ${chatWith.fullName}</strong> <span class="text-muted small">(${chatWith.role})</span></div>
      ...
      <form action="${pageContext.request.contextPath}/patient/chat" method="post" class="chat-input">
